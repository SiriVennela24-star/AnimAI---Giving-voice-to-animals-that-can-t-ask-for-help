...
METRO_SECTORS = [
    {"name": "Mumbai Sector A", "coords": [72.8777, 19.0760]},       # Mumbai
    {"name": "Delhi Sector B", "coords": [77.2090, 28.6139]},        # Delhi
    {"name": "Bengaluru Sector C", "coords": [77.5946, 12.9716]},     # Bengaluru
    {"name": "Kolkata Sector D", "coords": [88.3639, 22.5726]}        # Kolkata
]
...
async def analyze_incident(
    ...
    if audio:
        try:
            ...
            if len(audio_bytes) > 0:
                audio_labels, audio_telemetry = analyze_audio_distress(audio_bytes, audio.filename)
    ...
    if video:
        try:
            ...
            if len(video_bytes) > 0:
                processed_video, vision_labels, vision_conf = process_video_media(video_bytes, video.filename)
        ...
        try:
            ...
            if len(image_bytes) > 0:
                processed_img, vision_labels, vision_conf = process_image_media(image_bytes, image.filename)
    ...
    # Compile detected injuries/anomalies list
    detected_anomalies = []
    ...
    analysis_payload = AnalysisPayload(
        ...
        detected_injuries=detected_anomalies, # duplicate for backward-compatibility
        confidence_percentages=confidence_percentages
    )
    
    # DB Schema requirements: operational_status dynamically populated
...
async def list_incidents(db = Depends(get_database)):
    ...
    try:
        ...
        for record in records:
            # Convert BSON ObjectIDs to clean strings
            if "_id" in record:
                if not isinstance(record["_id"], str):
                    record["_id"] = str(record["_id"])
            
            if "incident_id" not in record or not record["incident_id"]:
                record["incident_id"] = record.get("_id", "")
                
            # Preserve operational_status if it exists, otherwise default to "Critical"
            if "operational_status" not in record or not record["operational_status"]:
                record["operational_status"] = "Critical"
            
            # Formulate analysis payload safely
            payload = record.get("analysis_payload", {})
            if isinstance(payload, dict):
                anomalies = payload.get("detected_anomalies")
                injuries = payload.get("detected_injuries")
                
                if anomalies is None and injuries is not None:
                    anomalies = injuries
                elif injuries is None and anomalies is not None:
                    injuries = anomalies
                elif anomalies is None and injuries is None:
                    anomalies = []
                    injuries = []
                    
                payload["detected_anomalies"] = anomalies
                payload["detected_injuries"] = injuries
...
async def create_community_report(body: CommunityReportRequest, db = Depends(get_database)):
    ...
    analysis_payload = AnalysisPayload(
        risk_score=risk_score,
        detected_anomalies=injuries,
        detected_injuries=injuries,
        confidence_percentages={"Text Report Confidence": 90.0}
    )

    # Convert this report into an incident. Set operational_status based on risk score dynamically
    if risk_score >= 75:
        status = "Critical"
    elif risk_score >= 40:
        status = "Warning"
...
async def predict_outbreaks(db = Depends(get_database)):
    ...
    for sec in sectors:
        density_bonus = sector_densities[sec] * 3
        predictions = []
        for i in range(7):
...
async def update_incident_status(incident_id: str, body: UpdateStatusRequest, db = Depends(get_database)):
    """
    ...
    try:
        result = await db["incidents"].update_one(
            {"incident_id": incident_id},
            {"$set": {"operational_status": body.operational_status}}
        )
        ...
        logger.info(f"Updated operational status of incident {incident_id} to {body.operational_status}")
        return {"status": "success", "incident_id": incident_id, "operational_status": body.operational_status}
